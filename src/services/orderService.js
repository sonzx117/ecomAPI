    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (!id) {
                ...
                if (order.image) {
                    order.image = new Buffer.from(order.image, 'base64').toString('binary')
                }
                ...
                for (let i = 0; i < orderDetail.length; i++) {
                    ...
                    for (let j = 0; j < orderDetail[i].productImage.length; j++) {
                        orderDetail[i].productImage[j].image = new Buffer.from(orderDetail[i].productImage[j].image, 'base64').toString('binary')
                    }
                }
                    ...
                    for (let j = 0; j < addressUser[i].order.length; j++) {
                        ...
                        for (let k = 0; k < orderDetail.length; k++) {
                            ...
                            for (let f = 0; f < orderDetail[k].productImage.length; f++) {
                                orderDetail[k].productImage[f].image = new Buffer.from(orderDetail[k].productImage[f].image, 'base64').toString('binary')
                            }
                        }
